...
> ℹ️ **無料お試しにはIP単位の上限もある（通常は何もしなくてよい）**
> Cookie単位の3件とは別に、**同一IPアドレスから1日60件**という保険がかかっています
> （`wrangler.jsonc` の `vars.FREE_TRIAL_IP_LIMIT`）。Cookieを消して何度も使う行為を抑えるためのものです。
>
> 会場のWi-Fiや携帯キャリアのCGNATでは、無関係な多数の人が同じグローバルIPを共有します。
> そのため**この上限は日付ごとにリセット**され、共有回線を恒久的にブロックしません。
> 1人3件なら20人が同じ回線で試せる計算なので、通常のイベント規模なら設定変更は不要です。
>
> - [ ] 40人を超える規模で一斉に触ってもらう場合のみ、`FREE_TRIAL_IP_LIMIT` を引き上げて再デプロイ（例: `"200"`）
> - [ ] この上限に当たった人には「同じ回線からのお試しが本日の上限に達しました」と**専用の文言**が出ます。
>       「3件使い切りました」とは別のメッセージなので、当日この文言を見たら回線側の上限だと判断できます。
...
- [ ] アプリ内の「お支払い方法の変更・解約」から Customer Portal に遷移できる
...
| 解約したい | マイページ > ご契約 > お支払い方法の変更・解約 から。**解約後も次回請求日まではそのまま使える**（Stripeを「請求期間の終了時にキャンセル」で設定しているため）旨を伝える |
